...
import { botDb } from "./src/utils/db.js";
...
const startWhatsApp = async () => {
  ...
  try {
    ...
    const settings = await botDb.getSettings();
    console.log(`✅ Bot settings loaded: ${settings.botName}`);
  } catch (error) {
  ...
  whatsapp.ev.process(async (events) => {
    if (events["connection.update"]) {
      ...
      if (connection === "open") {
        ...
        // Update bot settings dengan owner number
        try {
          const settings = await botDb.getSettings();
          if (!settings.ownerNumber) {
            await botDb.updateSettings({
              ownerNumber: process.env.OWNER || whatsapp.user.id.split(":")[0],
            });
      ...
      for (let msg of upsert.messages) {
        ...
        if (upsert.type == "notify") {
          ...
          if (processedMessage.isGroup) {
            ...
            if (store) {
              const metadata = await whatsapp.groupMetadata(
    ...
    if (events["groups.upsert"]) {
      ...
      for (const groupMetadata of newGroups) {
        try {
          ...
          // Save to database
          await botDb.getGroup(groupMetadata.id);
        } catch (error) {
          console.error(
